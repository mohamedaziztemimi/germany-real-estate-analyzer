...
export const propertyPayloadSchema = z.object({
  ...
  floor: z.number().int(),
  condition: z.enum(["poor", "medium", "good"]).nullable().optional(),
  has_elevator: z.number().int().min(0).max(1),
  has_balcony: z.number().int().min(0).max(1),
  energy_efficiency_class: z.enum(["A+", "A", "B", "C", "D", "E", "F"]),
  price_buy: z.number().positive("Purchase price must be positive"),
  reno_cost: z.number().min(0),
  price_per_m2: z.number().positive(),
  reno_cost_per_m2: z.number().min(0),
  uplift_pct: z.number(),
